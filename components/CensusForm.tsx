  ...
  const validateStep = (currentStep: number, data: CensusData): string | null => {
    ...
    if (currentStep === 1) {
      ...
      if (data.maritalStatus !== 'Single' && ageNum < 16) {
        return `Legal marriage age in HK is 16+. You entered age ${ageNum}.`;
      }
      
      const isHigherEd = data.education.includes('Bachelor') || data.education.includes('Master') || data.education.includes('Doctorate');
      if (isHigherEd && ageNum < 18) {
        return `It is unlikely to hold a higher degree at age ${ageNum}. Please check Age or Education level.`;
      }
    }
    ...
    if (currentStep === 2) {
      ...
      if (!data.occupation.trim()) return "Please enter your occupation (or 'Student'/'Retired').";
  ...
  const steps = [
    {
      ...
      content: (
        <div className="animate-in slide-in-from-right duration-300">
          ...
          <InsetGroup>
             <ListInput label="Full Name" name='fullName' value={formData.fullName} onChange={v => updateField('fullName', v)} placeholder="Chan Tai Man" />
             <ListInput label="HKID" name='hkid' value={formData.hkid} onChange={v => updateField('hkid', v)} placeholder="A123456(7)" />
             <ListSelect label="Gender" name='gender' value={formData.gender} onChange={v => updateField('gender', v as 'Male' | 'Female' | 'Other')} options={['Male', 'Female', 'Other']} />
             <ListInput label="Age" name='age' type="number" value={formData.age} onChange={v => updateField('age', v)} placeholder="e.g. 35" />
             <ListSelect label="District" name='district' value={formData.district} onChange={v => updateField('district', v)} options={DISTRICTS} last />
          </InsetGroup>
        </div>
      )
    },
    {
      ...
      content: (
        <div className="animate-in slide-in-from-right duration-300">
          ...
          <InsetGroup>
             <ListSelect label="Ethnicity" name='ethnicity' value={formData.ethnicity} onChange={v => updateField('ethnicity', v)} options={ETHNICITIES} />
             <ListSelect label="Education" name='education' value={formData.education} onChange={v => updateField('education', v)} options={EDUCATION_LEVELS} />
             <ListSelect label="Marital Status" name='maritalStatus' value={formData.maritalStatus} onChange={v => updateField('maritalStatus', v)} options={['Single', 'Married', 'Divorced', 'Widowed']} last />
          </InsetGroup>
        </div>
      )
    },
    {
      ...
      content: (
        <div className="animate-in slide-in-from-right duration-300">
          <SectionHeader title="Employment & Housing" footer="If you are retired, a student, or unemployed, please select the most appropriate option or 'Other'." />
          <InsetGroup>
             <ListSelect label="Industry" name='industry' value={formData.industry} onChange={v => updateField('industry', v)} options={INDUSTRIES} />
             <ListInput label="Occupation" name='occupation' value={formData.occupation} onChange={v => updateField('occupation', v)} placeholder="Engineer, Student, Retired..." />
             <ListSelect label="Housing Type" name='housingType' value={formData.housingType} onChange={v => updateField('housingType', v)} options={HOUSING_TYPES} last />
          </InsetGroup>
        </div>
      )
    },
    {
      ...
      content: (
        <div className="animate-in slide-in-from-right duration-300">
           ...
           <InsetGroup>
             <ListSelect label="Migration Status" name='migrationStatus' value={formData.migrationStatus} onChange={v => updateField('migrationStatus', v)} options={MIGRATION_STATUS} last />
           </InsetGroup>
